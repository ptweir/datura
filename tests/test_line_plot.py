...
df = pd.DataFrame(data={'x':[1, 2], 'y1':[-1, 1], 'y2':[0, -1], 'y1u':[0, 3], 'y1l':[-2, -1]})
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (np.hstack((ar[:, :1], ar[:, :1])), ar[:,1:3], None, None, 'two_lines')
    ],
)
class TestLinePlot:
    ...
    def test_axis_limits_from_x_ticks(self, xs, ys, yus, yls, filename_base):
        ...
        _out = datura.plot(xs, ys, yus=yus, yls=yls, x_ticks=[0, 1, 2, 3], filename=out_svg)
        with open(out_svg) as out_svg_file:
            with open(persistent_svg) as persistent_svg_file:
                assert out_svg_file.read() == persistent_svg_file.read()
    ...
    def test_axis_limits_from_y_ticks(self, xs, ys, yus, yls, filename_base):
        ...
        out_svg = os.path.join('tests', this_filename_base+'.svg')
        persistent_svg = os.path.join('tests', this_filename_base+'_persistent.svg')
        _out = datura.plot(xs, ys, yus=yus, yls=yls, y_ticks=[0, 1, 2], filename=out_svg)
        with open(out_svg) as out_svg_file:
            with open(persistent_svg) as persistent_svg_file:
                assert out_svg_file.read() == persistent_svg_file.read()
