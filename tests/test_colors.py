...
def test_fill_colors():
    ...
    df = pd.DataFrame(data={'x':dts, 'y':ar[:, 1], 'yu':ar[:, 2], 'yl':ar[:, 0]})

    this_filename_base = 'colors'
    ...
    _out = datura.plot(df['x'], df['y'], yus=df['yu'], yls=df['yl'],
                       colors=['midnightblue'], fill_colors=['palegreen'], fill_opacities=[0.3],
                       filename=out_svg, y_ticks=[0, 50])
...
def test_many_colors():
    ...
    this_filename_base = 'many_colors'
    out_svg = os.path.join('tests', this_filename_base+'.svg')
    persistent_svg = os.path.join('tests', this_filename_base+'_persistent.svg')

    _out = datura.plot(dts, ar, filename=out_svg, labels=labels, y_ticks=[0, 10, 20, 30, 40])
    with open(out_svg) as out_svg_file:
        with open(persistent_svg) as persistent_svg_file:
            assert out_svg_file.read() == persistent_svg_file.read()